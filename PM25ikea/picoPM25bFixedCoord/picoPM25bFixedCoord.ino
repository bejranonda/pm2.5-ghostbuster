...
#define MQTTUSER "pm25"
#define MQTTPASS "FGsldku3ir"

// Parameters
const unsigned int age_minimum = 60000; // Time in ms
int mindist = 50;		// minimum distance in meters before 't'
...
const int version = 301;

#define BASETOPIC	"pm25"	// no trailing slash!
#define RECONNECTINTERVAL	60000 // WiFi 300000			// ms (5m)
#define STORE		"/data/owntracks.log"
//#define PINGINTERVAL		(60 * 60 * 1000)		// ms
#define PINGINTERVAL		(900 * 1000)		// ms Time to get PM25 again
...
void setup()
{
  ...
  Serial.print("deviceID=");
	...
	snprintf(pubtopic, sizeof(pubtopic), "%s/%s/air", BASETOPIC, deviceID);
  ...
  if (!wifiManager.autoConnect("PM2.5")) {
...
//  }
  ...
  nss.listen();
	...
	// if (gps.location.isValid() && gps.location.age() < age_minimum) {
  if (true) {
		if (counter % 10 == 0) {
			// gpstime_set(gps);
			timeClient.update();
      ...
      last_lat = gps_fix_lat;
			last_lon = gps_fix_lon;
      Serial.println("serialize gps f");
		} else {
			if (now - pingInterval >= PINGINTERVAL) {
				serialize(gps, 'p');
				// pingInterval = millis();
        Serial.println("serialize gps p");
			}
      ...
      unsigned long meters = 0;
			...
			if (meters_since_pub >= mindist) {
				serialize(gps, 'v');
        Serial.println("serialize gps v");
				meters_since_pub = 0;
			}
			last_lat = gps_fix_lat;
			last_lon = gps_fix_lat;
		}
...
static void serialize(TinyGPSPlus &gps, char t)
{
  ...
  if(SerialCom::handleUart(state)){
    ...
    pingInterval = millis(); // Update last reading
  }
